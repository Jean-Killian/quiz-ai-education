            ...
            <div class="flex justify-between items-center mb-8 theme-bg-panel p-4 border-2 theme-border-primary theme-glow rounded relative overflow-hidden">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div class="text-xs font-black theme-primary uppercase">{{ auth()->user()->name }}</div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4 text-right">
                    <div>
                        ...
                        <div class="text-xs font-black text-white uppercase">
                            {{ auth()->id() === $duel->challenger_id ? $duel->defender->name : $duel->challenger->name }}
                        </div>
                    </div>
                    <div class="size-12 rounded border theme-border overflow-hidden">
                        <img src="{{ auth()->id() === $duel->challenger_id ? $duel->defender->avatar_url : $duel->challenger->avatar_url }}" alt="" class="w-full h-full object-cover">
                    </div>
                </div>
            </div>
            ...
            <div class="theme-bg-panel border theme-border rounded p-8 min-h-[400px] flex flex-col justify-center relative overflow-hidden">
                ...
                <div id="quiz-content">
                    @foreach($quiz->questions as $index => $question)
                        <div class="question-step hidden" data-index="{{ $index }}" data-id="{{ $question->id }}">
                            <div class="mb-8">
                                <span class="theme-primary text-xs font-black mb-2 block">> Q_{{ $index + 1 }} / {{ $quiz->questions->count() }}</span>
                            ...
                            <div class="grid grid-cols-1 gap-3">
                                @foreach($question->answers as $answer)
                                    <button class="answer-btn text-left p-4 theme-bg-deep border theme-border rounded hover:theme-border-primary hover:theme-bg-panel transition-all group flex justify-between items-center" 
                                        ...
                                        <span class="text-slate-300 group-hover:text-white transition-colors">{{ $answer->answer_text }}</span>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function finishDuel() {
                ...
                fetch("{{ route('duels.submit', $duel->id) }}", {
                    method: 'POST',
                    headers: {
                    ...
                    window.location.href = "{{ route('duels.result', $duel->id) }}";
                });
            }
        });
    </script>
